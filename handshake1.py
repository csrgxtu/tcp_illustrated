...
import logging
import random
from scapy.all import IP, Net, TCP, send, sniff, conf
...
class TcpHandshake(object):
    ...
    def _sr1(self, pkt):
        send(pkt)
        ans = sniff(
            filter="tcp port %s" % self.target[1],
            lfilter=self.match_packet,
            count=1,
            timeout=1
        )
    ...
    def handle_recv(self, pkt):
        if pkt and pkt.haslayer(IP) and pkt.haslayer(TCP):
            if pkt[TCP].flags & 0x3f == 0x12:   # SYN+ACK
                ...
                logger.info("server: {} {}".format(pkt[TCP].seq, pkt[TCP].ack))
    ...
    def send_syn(self):
        ...
        logger.info('client:send_syn: {}:{}-->{}-->{}:{}'.format(
            self.src,
            self.sport,
            self.l4[TCP].seq,
            self.target,
    ...
    def send_synack_ack(self, pkt):
        ...
        logger.info('client:send_syn_and_ack: {}:{}-->{}-->{}:{}'.format(
            self.src,
            self.sport,
            self.l4[TCP].ack,
            self.target,
